...
func (pe *PEFile) parseImports64(importDesc *ImportDescriptor) (err error) {
	var table, ilt, iat []ThunkData64
	if importDesc.Data.Characteristics > 0 {
		ilt, err = pe.getImportTable64(importDesc.Data.Characteristics, importDesc)
		if err != nil {
			log.Println("ILT (ImportDescriptor.Characteristics) parse error", err)
		}
	}
	if importDesc.Data.FirstThunk > 0 {
		iat, err = pe.getImportTable64(importDesc.Data.FirstThunk, importDesc)
		if err != nil {
			log.Println("IAT (ImportDescriptor.FirstThunk) parse error", err)
			//return err
		}
	...
	impOffset := uint32(0x4)
	addressMask := uint64(0x7fffffffffffffff)
	ordinalFlag := uint64(IMAGE_ORDINAL_FLAG)

	numInvalid := uint32(0)

	for idx, thunk := range table {
		var imp ImportData64
		imp.StructTable = thunk
		imp.OrdinalOffset = uint64(thunk.FileOffset)
		...
		if thunk.Data.AddressOfData > 0 {
			...
			if table[idx].Data.AddressOfData&ordinalFlag > 0 {
				imp.ImportByOrdinal = true
				imp.Ordinal = table[idx].Data.AddressOfData & uint64(0xffffffff)
			} else {
				imp.ImportByOrdinal = false
				imp.HintNameTableRva = table[idx].Data.AddressOfData & addressMask

				if imp.HintNameTableRva > 0 {
					if err := pe.readRVA(&imp.Hint, uint32(imp.HintNameTableRva)); err != nil {
						return err
					}
				}
				...
				nameOffsetTmp, err := pe.getOffsetFromRva(uint32(table[idx].Data.AddressOfData + 2))
				imp.NameOffset = uint64(nameOffsetTmp)
				if err != nil {
					return err
				}
			}
			imp.ThunkOffset = uint64(table[idx].FileOffset)
			imp.ThunkRva = uint64(pe.getRvaFromOffset(uint32(imp.ThunkOffset)))
		}

		imp.Address = uint64(importDesc.Data.FirstThunk + pe.OptionalHeader.Data.ImageBase + (uint32(idx) * impOffset))
		...
		if imp.Ordinal > 0 || hasName {
			importDesc.Imports64 = append(importDesc.Imports64, imp)
		}
	}
...
func (pe *PEFile) getImportTable64(startRVA uint32, importDesc *ImportDescriptor) ([]ThunkData64, error) {
	// Setup variables
	thunkTable := make(map[uint32]ThunkData64)
	...
	for rva := startRVA; rva < lastAddr; {
		...
		thunk := newThunkData64(thunkOffset)
		if err := pe.readOffset(&thunk.Data, thunk.FileOffset); err != nil {
			msg := fmt.Sprintf("Error Parsing the import table.\nInvalid data at RVA: 0x%x", rva)
			log.Println(msg)
			return nil, errors.New(msg)
		...
		// Seen in PE with SHA256:
		// 5945bb6f0ac879ddf61b1c284f3b8d20c06b228e75ae4f571fa87f5b9512902c
		if thunk.Data.AddressOfData >= uint64(startRVA) && thunk.Data.AddressOfData <= uint64(rva) {
			log.Printf("Error parsing the import table. "+
				"AddressOfData overlaps with THUNK_DATA for THUNK at:\n  "+
				"RVA 0x%x", rva)
			break
		...
		if thunk.Data.AddressOfData > 0 {
			// If the entry looks like could be an ordinal...
			if thunk.Data.AddressOfData&uint64(ordinalFlag) > 0 {
				// but its value is beyond 2^16, we will assume it's a
				// corrupted and ignore it altogether
				if uint32(thunk.Data.AddressOfData)&uint32(0x7fffffff) > uint32(0xffff) {
					msg := fmt.Sprintf("Corruption detected in thunk data at 0x%x", rva)
					log.Printf(msg)
					return nil, errors.New(msg)
				}
				...
				if uint32(thunk.Data.AddressOfData) > maxAddressOfData {
					maxAddressOfData = uint32(thunk.Data.AddressOfData)
				}
				if uint32(thunk.Data.AddressOfData) < minAddressOfData {
					minAddressOfData = uint32(thunk.Data.AddressOfData)
				}
			}
		}
